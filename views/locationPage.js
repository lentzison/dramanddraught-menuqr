  ...
  return `
    ...
    <head>
      ...
      <style>
        ...
        /* Secondary links: refined dark glass with a gold accent border. */
    ...
    <body>
      ...
      <main class="loc-content-flow">
        ...
        <section class="qr-actions container" aria-label="Quick actions">
          <div class="linktree">
            ${buttons.map((l, i) => `<a class="${actionButtonClass(l)} stagger" href="${l.url}"${l.url.startsWith('/') ? '' : ' target="_blank" rel="noopener noreferrer"'} style="animation-delay:${Math.min(i * 0.04, 0.2)}s;"><span>${escHTML(l.label)}</span></a>`).join('')}
          </div>
        </section>
